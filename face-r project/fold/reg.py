...
from EncodeGenerator import encoding
"""sys.path.append('..')
from temp import tempu
tempu()"""
...
def register_student():
    ...
    if image_path:
        ...
        folderPath = 'old'
        fileName = f'{folderPath}/{student_id}.png'  # Corrected the file name


        bucket = storage.bucket()
        blob = bucket.blob(fileName)
        blob.upload_from_filename(os.path.join(images_dir, f"{student_id}.png"))  # Upload the image
...
def clear_inputs():
    for entry_field in entry_fields:
        if isinstance(entry_field, tk.Entry):
            entry_field.delete(0, 'end')
        elif isinstance(entry_field, tk.StringVar):
            entry_field.set("")  # Clear the StringVar
...
def panel():
        root.destroy()
        os.system('python fold/panel.py')
...
title_label = tk.Label(root, text="Student Registration", font=("Helvetica", 20), bg="#b0b097")
...
for i, label_text in enumerate(labels):
    label = tk.Label(root, text=label_text,font=("Helvetica", 12), bg="#b0b097")
...
tk.Label(root, text="Last Attendance Time:",font=("Helvetica", 12), bg="#b0b097").grid(row=len(labels)+1, column=0, padx=10, pady=5, sticky="w")
...
register_button.grid(row=len(labels)+2, column=1, padx=10, pady=10)

# Create buttons to browse and capture image
browse_button = tk.Button(root, text="Browse Image", command=browse_image)
browse_button.grid(row=len(labels)+3, column=0, padx=10, pady=5)
# Create a button to launch the panel
panel_button = tk.Button(root, text="Panel", command=panel)
panel_button.grid(row=len(labels)+3, column=1, padx=10, pady=10)

capture_button = tk.Button(root, text="Capture Image", command=capture_image)
capture_button.grid(row=len(labels)+3, column=2, padx=10, pady=5)

# Create button to reset inputs
clear_button = tk.Button(root, text="Reset", command=clear_inputs)
clear_button.grid(row=len(labels)+4, column=1, padx=10, pady=5)


# Create label to display the image
image_label = tk.Label(root)
image_label.grid(row=len(labels)+6, column=0, columnspan=2, padx=10, pady=5)
